...
SDI built a Make-Store-Pack simulation using our breakthrough [Discrete Rate Simulation](https://en.wikipedia.org/wiki/Discrete_rate_simulation) technology — the same innovation that originated from our work on a leading commercial DES platform's core libraries. The database system that powers these models was also designed by SDI's founder, and has been [reimplemented as an open platform](https://db.chiaha.com/). The rate-based simulation engine that runs on it has been rebuilt as well — and the results have surprised even us. DRS proved perfect for studying different scheduling and warehouse stocking policies under the new constraints.
...
**Additional Examples:**
- **[Chemical Manufacturing](/manufacturing/case-studies/#chemical-packaging-model):** SDI helped Rohm & Haas (Dow Chemical) evaluate alternative packaging line configurations and scheduling parameters, uncovering "additional options for low capital cost business growth, some of which were not readily apparent beforehand." *— Rick Dougherty, Senior Manufacturing Analyst*
